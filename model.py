...
def preprocess_reaction_for_training(reaction_tuple, 
    ...
    # Create product mask - accounting for conditioning strategy
    product_mask = [False] * config.max_seq_length
    
    # Find where products start (this varies by strategy)
    if conditioning_strategy == "single":
        product_start = 1 + len(react_ids) + 1 + len(agents_ids) + 1
    elif conditioning_strategy == "dual":
        product_start = 1 + len(react_ids) + 1 + len(agents_ids) + 1 + 1  # +1 for extra class token
    elif conditioning_strategy == "prefix":
        product_start = 3 + len(react_ids) + 1 + len(agents_ids) + 1  # +3 for [CLASS] class [SEP]
    elif conditioning_strategy == "interleaved":
        product_start = 1 + len(react_ids) + 1 + 1 + len(agents_ids) + 1 + 1  # +2 for extra class tokens
    
    # Set product mask
    for i in range(product_start, min(product_start + len(prod_ids), config.max_seq_length)):
        product_mask[i] = True
...
class ReactionMD4(nn.Module):
    ...
    def __init__(self, config: ReactionMD4Config):
        ...
        # Each token ID is mapped to a d_model-dimensional embedding (+1 because mask_token_id is included in the embedding table)
    ...
    def forward(self, input_ids: torch.Tensor, t: torch.Tensor,
                ...
                product_mask: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        """
        ...
        # Compute loss during training (only on product positions)
        ...
        if target_ids is not None and product_mask is not None:
            ...
            if product_mask.any():
                ...
                # valid_positions = product_mask & (target_ids != self.config.pad_token_id)
...
def generate_examples_during_training(model: ReactionMD4, dataset, tokenizer, num_examples: int = 3):
